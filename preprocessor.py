...
ID_COLUMN_PATTERNS = [
    "record_id", "id", "_id", "index", "uuid", "user_id", "patient_id", "employee_id",
    "consumer_id", "meter_id", "transformer_id", "feeder_id", "substation_id"
]

TARGET_LABEL_PATTERNS = [
    "unreliable_record_label", "label_description", "unreliable_record_category",
    "expected_correction_rule", "cpri_benchmark_detection_status", "is_anomaly",
    "label", "target", "ground_truth", "anomaly_label", "class", "is_unreliable"
]
...
class DataPreprocessor:
    ...
    def is_id_column(self, col_name: str) -> bool:
        ...
        return name_lower in ID_COLUMN_PATTERNS or name_lower.endswith("_id") or name_lower.startswith("id_") or name_lower.endswith("_code")

    def is_target_label_column(self, col_name: str) -> bool:
        """Determines if a column name represents a benchmark label or target field."""
        name_lower = col_name.lower().strip()
        return name_lower in TARGET_LABEL_PATTERNS or "label" in name_lower or "benchmark" in name_lower or name_lower.startswith("target_")
    ...
    def extract_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 3. Dynamic numeric feature extraction (excluding ID and Label columns)
        candidate_cols = []
        sub_df = pd.DataFrame()

        for col in df.columns:
            if self.is_id_column(col) or self.is_target_label_column(col):
                continue
        ...
        # 4. Fallback if no non-ID/non-label numeric columns found: take all numeric non-ID non-label or all numeric
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
        cols_to_use = [c for c in numeric_cols if not self.is_id_column(c) and not self.is_target_label_column(c)]
        if not cols_to_use:
            cols_to_use = [c for c in numeric_cols if not self.is_id_column(c)]
        if not cols_to_use:
